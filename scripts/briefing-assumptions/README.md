...
(pure logic vs mocks) and not smoke tests (broad-and-shallow). `09-policy-calendar.py` is the one
deliberate exception: it is pure logic with no external service at all, and it lives here anyway
because the thing it guards — the hardcoded `POLICY_CALENDAR` — is a runtime assumption about the
outside world that nothing else in the system can detect going wrong.
...
and 06 from a GitHub runner on a weekly schedule** (`0 16 * * 1`) as well as on dispatch, each as its own
...
in requirements.txt — dev-only deps). Tests 5, 7, 8 and 9 need neither a key nor extra deps, and 9
