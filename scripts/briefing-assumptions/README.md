...
| `14-wikipedia-ua.py` | W1–W3: **the Wikipedia User-Agent is policy-compliant AND survives a burst.** Owen's Alphabet Soup went dark at the end of August 2026 because `WIKI_UA` ended in a bare `https://github.com/` — a hostname with no repo path and no contact, which Wikimedia treats as unidentified. The trap is that it is a rate-limit CLASS, not a block: a slow one-off fetch still succeeds (which is how the first diagnosis briefly went wrong), but the lesson leg fetches in a tight burst and under burst the generic UA measured 429 on 12/12 while a compliant one measured 200 on 12/12. W1 checks the string shape offline so it goes red even when Wikipedia is unreachable; W2 fires 8 distinct titles with no sleeps; W3 re-runs the burst with the OLD generic UA as a control, so a W2 pass means "we are not throttled" rather than "nobody is throttled today" — and W3 is reported, never failed on, because it measures Wikimedia's policy and not our code | network (no key) |
