...
| Test | Proves | Needs |
|---|---|---|
| `04-external-boundary-smoke.py` | RSS feeds parse + enough fresh items in 72h + ≥1 world feed alive; Wikipedia S&P500/NDX tables parse to ~500/~100 **through production's own parser** (`scripts.data.constituents` — imported, not re-implemented: the previous `pandas.read_html` copy could not run in CI at all, and was blind to the cell-shape drift the shipping regex fails on); symbol normalization output is stable | nothing (runnable now); `feedparser` |
...
| `13-us-news-editorial.py` | C1–C4: **the US national section reports events and outcomes, never the partisan contest about them.** This section re-admits a category `summarize.SYSTEM` excluded outright until 2026-08-20, so the narrower rule that replaced the exclusion gets a machine. **C1 is the load-bearing half**: it fires the contest detector at seeded event copy and seeded contest copy and fails if either side is wrong — without it C3/C4 could pass forever by matching nothing, which is how a guard rots. C2 asserts the rule is still in `summarize.SYSTEM`. C3 makes a live call over real AP/NPR candidates and fails if a returned `us` item carries contest vocabulary (skipped, not failed, without `GEMINI_API_KEY`). C4 re-scans shipped archive editions. **Three carve-outs, each pinned in BOTH directions by C1**: a party adjective qualifying a person's role is biography (`Former Republican congressman George Santos...`), `the polls` / `polling place` is a location not opinion polling, and `midterm elections` names a date the way `the November elections` does — while the plural collective (`Republicans said`), bare `polling`, and bare `the midterms` all still fire. C3 is **JUDGMENT-class**: it is scored against whatever the world published today, so it flaps by design and pages at low priority. All three false positives so far came from ONE civil-military story about troops at polling sites; a fourth is a signal to reconsider a keyword detector, not to cut a fourth hole in it | `GEMINI_API_KEY` for C3 only; C1, C2 and C4 need no key |
| `15-alert-classification.py` | A1–A3: **the data-smoke alarm names the right cause.** On 2026-08-31 a red run paged, at high priority, "A DATA SOURCE went red from the runner: external-boundary. Something is broken or has drifted upstream" — while every source was healthy. 04 had exited 3 (INFRA) on `import pandas`. The alert read only the step OUTCOME, and "failure" collapses exit 1 (assertion red), exit 2 (refused) and exit 3 (infra) into one verdict, so the one page this job exists to make trustworthy pointed at the wrong animal. A1 asserts the alarm still exists and is still gated on `failure() || cancelled()`; A2 asserts every test step captures its exit code AND that every captured code is read by the alert (a step graded by outcome alone can only ever be classified by the old ambiguous rule); **A3 executes the shipped classifier**, lifted out of the workflow rather than copied, over eight cases — most importantly that an exit 3 pages as a HARNESS fault and never as an upstream drift. Stdlib only, on purpose: importing a package CI does not have is the exact bug it exists to prevent recurring | nothing (runnable now, offline); needs `bash` |
| `14-wikipedia-ua.py` | W1–W4: **the Wikipedia User-Agent is policy-compliant, reaches every caller, AND survives a burst.** Owen's Alphabet Soup went dark at the end of August 2026 because `WIKI_UA` ended in a bare `https://github.com/` — a hostname with no repo path and no contact, which Wikimedia treats as unidentified. The trap is that it is a rate-limit CLASS, not a block: a slow one-off fetch still succeeds (which is how the first diagnosis briefly went wrong), but the lesson leg fetches in a tight burst and under burst the generic UA measured 429 on 12/12 while a compliant one measured 200 on 12/12. W1 checks the string shape offline so it goes red even when Wikipedia is unreachable; **W2 checks that every production module which actually fetches Wikipedia sends `config.WIKI_UA`** — added 2026-09-01 after the 08-31 fix was found to have reached `data/lessons.py` and left `data/constituents.py` (breadth's constituent scrape) on the generic `config.USER_AGENT`, a second instance of the same bug that W1 could not see because it only ever reads one string; W3 fires 8 distinct titles with no sleeps; W4 re-runs the burst with the OLD generic UA as a control, so a W3 pass means "we are not throttled" rather than "nobody is throttled today" — and W4 is reported, never failed on, because it measures Wikimedia's policy and not our code | network (no key) |
| `06-policy-relevance.py` | G1–G8: the model, given the production `LIFE_PROFILE` and a real candidate set, selects only documents that create a number/deadline/obligation; every citation joins back through the production `_norm_url` to a fetched document on one of the two primary `.gov` hosts; every item has a non-empty `effect`; it **rejects three seeded real-but-irrelevant decoys** (OSHA benzene, mine ventilation, blacksmith shops) so a model that just returns the first N cannot pass; invents no dollar amount or percentage absent from the source; writes no hedge word in a settled-law `effect`; two live tilde-bearing Utah bills round-trip; and **G8 asserts THE BATCH INVARIANT — production must never hand the model fewer documents than a batch.** G8 used to be two one-document probes closing the "1-of-1 vs best-2-of-24" seam, and it went RED on CI run 30851392524 (given ONE on-profile document in isolation the model returned nothing, while ranking a batch of 26 in the same run it selected correctly). The fix removed that mode from production — `policy.get_policy()` now sends the whole prefiltered window and the `policy_seen` dedupe runs after selection — so G8 now calls the SHIPPED `get_policy()` twice, the second time with every returned id marked seen, and goes red if the candidate set shrinks. The two one-document calls still run as a **recorded measurement** (`single_candidate_probe.single_candidate_selects`, printed as a NOTE, asserting nothing): the model limitation is real and worth tracking, and **the model did not get better**. Since 2026-09-01 they also cannot FAIL the run: a probe that raises is recorded as unavailable (NOTE + `null` in the fingerprint, distinct from "returned 0"). Until then a transport error in either probe exited 3, and on 2026-08-31 a Gemini `ClientError` did exactly that — reddening a run in which every real gate had passed, and paging at high priority. A measurement that asserts nothing about its answer must not be able to redden a run on its way to asserting nothing; the BATCH call is the load-bearing one and still exits 3. Everything production-shaped is IMPORTED (`policy._federal_candidates`, `summarize.POLICY_SYSTEM`, `PolicySection`, `_MONEY`, `_norm_url`, `_policy_docs_block`) — a rename goes red at import instead of quietly proving nothing. The one deliberate exception is the host allowlist, asserted against a local tuple so widening `_ALLOWED_HOSTS` cannot silently widen this gate | `GEMINI_API_KEY`; `google-genai`, `pydantic` |
...
| `POLICY_PROBE_FAIL_CONTROL=positive\|negative\|both` | 06 | forces the named single-candidate probe to raise. The test must still exit **0**, reporting the probe as unavailable. Pins the 2026-09-01 fix: before it, a raising probe exited 3 and reddened the run. Needs the key, since the batch call runs first |
...
`GEMINI_API_KEY` (tests 3 and 6). Test 1 is the only one needing a package outside
requirements.txt (`pandas` + `lxml`); it is excluded from CI anyway. Tests 4, 5, 7, 8, 9, 10 and 14
need neither a key nor extra deps, and 9 needs no network either. Test 11 is the one JavaScript gate — it runs the shipped `docs/app.js`
...
BRIEFING_SMOKE_ALLOW_DEV=true python scripts/briefing-assumptions/14-wikipedia-ua.py
```
