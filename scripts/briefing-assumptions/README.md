...
| `07-utah-bill-detail.py` | 3 rows sampled from the LIVE passed-bills page (≥1 HB, ≥1 SB) resolve via `urljoin` to absolute `https://le.utah.gov/` URLs; each bill's detail page returns 200 under the exact UA production sends; a stdlib tag-strip yields ≥200 chars that **contain the bill number and ≥2 title tokens** (content identity, not just length — 200 chars of nav chrome would pass a length-only check); body size and per-fetch wall time under stated ceilings | nothing (runnable now) |
