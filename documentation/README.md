...
- `operations.md` — scheduling and DST, one-time deployment, runtime behavior, monitoring and every
  alarm path (six `notify.py` publishers plus five workflow-level ntfy curls), run-state keys and
  their writers, failure modes and recovery, regression tests, and cost.
- `INDEX.json` — the machine-readable manifest. One small file an LLM can load to decide which doc
  to read next; it names this file as its `human_index`.
...
- Add or remove a followed ticker: edit `watchlist.txt` at the repo root. Nothing else changes.
...
- v6 (2026-09-09, morning) — the leveraged ETF pulse: SOXL, SPXL and TQQQ with RSI-14 and their
  1-month closing band, rendered under the must-knows and spoken in the same slot.
- v7 (2026-09-09, afternoon) — the pulse becomes the WATCHLIST, plus an edge-triggered ntfy push the
  moment a ticker crosses into buy or trim range: any Yahoo ticker via `watchlist.txt`, a
